  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-600 via-indigo-600 to-purple-800 p-4 md:p-8">
      <div className="w-full max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-800">{userDetail.username}</h1>
        ...
        {/* Answer Statistics */}
        <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">📊 Thống kê đáp án</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-green-50 p-6 rounded-lg border-2 border-green-300">
              <p className="text-sm font-semibold text-gray-600 uppercase mb-2">✅ Trả lời đúng</p>
              <p className="text-4xl font-bold text-green-600">{answerStats.correct}</p>
              <p className="text-sm text-gray-500 mt-1">/ {answerStats.total} câu</p>
            </div>
            <div className="bg-red-50 p-6 rounded-lg border-2 border-red-300">
              <p className="text-sm font-semibold text-gray-600 uppercase mb-2">❌ Trả lời sai</p>
              <p className="text-4xl font-bold text-red-600">{answerStats.wrong}</p>
              <p className="text-sm text-gray-500 mt-1">{((answerStats.wrong / answerStats.total) * 100).toFixed(1)}%</p>
            </div>
            <div className="bg-yellow-50 p-6 rounded-lg border-2 border-yellow-300">
              <p className="text-sm font-semibold text-gray-600 uppercase mb-2">⏭️ Không trả lời</p>
              <p className="text-4xl font-bold text-yellow-600">{answerStats.notAnswered}</p>
          ...
          <div className="bg-white rounded-3xl shadow-2xl p-8 mb-8">
            ...
            <div className="space-y-6">
              {wrongAnswers.map((wrong, idx) => (
                <div key={idx} className="bg-gradient-to-r from-red-50 to-orange-50 border-3 border-red-300 rounded-lg p-6">
                  ...
                  <p className="text-lg font-semibold text-gray-800 mb-6 p-4 bg-white rounded-lg border-l-4 border-indigo-600">
                    {wrong.question}
                  </p>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div className="bg-white p-4 rounded-lg border-3 border-red-400">
                      <p className="text-xs font-bold text-red-600 uppercase mb-2 tracking-wider">❌ Câu trả lời của bạn:</p>
                      <p className="text-gray-800 font-semibold text-base">{wrong.userAnswer}</p>
                    </div>
                    <div className="bg-white p-4 rounded-lg border-3 border-green-400">
                      <p className="text-xs font-bold text-green-600 uppercase mb-2 tracking-wider">✅ Đáp án đúng:</p>
                      <p className="text-gray-800 font-semibold text-base">{wrong.correctAnswer}</p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {wrongAnswers.length === 0 && (
          <div className="bg-white rounded-3xl shadow-2xl p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-green-600 mb-2">Hoàn hảo!</h2>
